...
MODALITY_INSTRUCTIONS = {
    "Bone X-Ray": {
        "technique": (
            "Trace the cortical outlines of every visible bone from end to"
            " end. Look specifically for discontinuities, sharp steps,"
            " angulations, or displaced fragments."
        ),
        "checklist": (
            "Fractures, Osteoporosis signs, Bone tumors, Arthritis (joint space"
            " narrowing), and Osteomyelitis."
        ),
    },
    "Chest X-Ray": {
        "technique": (
            "Use the ABCDE approach: Airway (trachea midline), Breathing (lung"
            " fields clear, inspect pleural margins for air/fluid), Circulation"
            " (heart size and borders), Disability (rib fractures), Everything"
            " else (diaphragm contours)."
        ),
        "checklist": (
            "Pneumonia, Tuberculosis, Lung cancer, COVID-19 pneumonia, Pleural"
            " effusion, Pneumothorax, COPD, Cardiomegaly, and Pulmonary edema."
        ),
    },
    "Brain CT": {
        "technique": (
            "Evaluate for symmetry. Check for hyperdense areas (acute"
            " bleeding), hypodense areas (ischemia/edema), midline shift, mass"
            " effect, and ventricular effacement."
        ),
        "checklist": (
            "Stroke signs, Brain bleed (hemorrhage), Tumors, Trauma/skull"
            " fractures, and Hydrocephalus."
        ),
    },
    "Abdominal X-Ray": {
        "technique": (
            "Assess bowel gas patterns (look for dilated loops or air-fluid"
            " levels), solid organ outlines, and look for abnormal"
            " calcifications or free air under the diaphragm."
        ),
        "checklist": (
            "Bowel obstruction, Kidney stones, Perforation (pneumoperitoneum),"
            " and Constipation."
        ),
    },
    "Dental X-Ray": {
        "technique": (
            "Examine the enamel cap, dentin, and pulp chamber of each tooth."
            " Assess the alveolar bone levels and the periodontal ligament"
            " space."
        ),
        "checklist": (
            "Cavities (radiolucencies), Impacted teeth, Jaw"
            " infection/abscesses, and Bone loss."
        ),
    },
    "Chest CT": {
        "technique": (
            "Evaluate lung parenchyma using lung windows, check mediastinal"
            " structures using soft tissue windows, and assess for pulmonary"
            " emboli or masses."
        ),
        "checklist": (
            "Lung nodules, Cancer, Fibrosis, Pulmonary embolism, and Severe"
            " infection."
        ),
    },
    "Abdominal CT": {
        "technique": (
            "Perform a systematic review of solid organs (liver, spleen,"
            " kidneys, pancreas), hollow viscus, vessels, and look for free"
            " fluid or lymphadenopathy."
        ),
        "checklist": (
            "Liver disease, Pancreatitis, Kidney stones, Appendicitis, and"
            " Tumors."
        ),
    },
    "Spine CT": {
        "technique": (
            "Evaluate vertebral body alignment, disk spaces, facet joints, and"
            " the spinal canal for narrowing or impingement."
        ),
        "checklist": "Disc disease, Spinal fractures, and Compression.",
    },
    "Mammography": {
        "technique": (
            "Compare bilateral symmetry. Search for spiculated masses,"
            " architectural distortion, and clusters of microcalcifications."
        ),
        "checklist": (
            "Breast cancer, Calcifications, Cysts, and Dense tissue"
            " abnormalities."
        ),
    },
    "Unknown": {
        "technique": (
            "Perform a systematic visual sweep of the entire image from"
            " outside to inside, noting any asymmetries, abnormal densities,"
            " or structural disruptions."
        ),
        "checklist": (
            "Any visible anatomical abnormalities or signs of trauma."
        ),
    },
}
...
class MedicalGraphEngine:
  ...
  def _triage_node(self, state: MedicalAgentState) -> dict:
    triage_structured_llm = self.vision_llm.with_structured_output(
        TriageClassification
    )

    system_prompt = (
        "You are an expert medical triage AI. Analyze the uploaded image and"
        " categorize it strictly into one of the designated medical imaging"
        " modalities."
    )
  ...
  def _specialist_node(self, state: MedicalAgentState) -> dict:
    ...
    instructions = MODALITY_INSTRUCTIONS.get(
    ...
    system_prompt = (
        f"You are a highly vigilant, expert radiologist specializing in"
        f" {modality}.\n"
        f"CRITICAL INSTRUCTION: Do NOT assume this image is normal. You must"
        f" actively search for pathology using this specific radiological"
        f" method:\n"
    ...
    response = self.vision_llm.invoke(
        [{"role": "system", "content": system_prompt}, image_message]
    )
  ...
  def _qa_node(self, state: MedicalAgentState) -> dict:
    ...
    history = state.get("clinical_metadata", {}).get(
        "diagnostic_summary", "No prior analysis found."
    )
    ...
    system_prompt = (
        ...
        f"1. You are in CHAT MODE. DO NOT generate a new radiologist report. DO"
        f" NOT output 'Findings' or 'Image Description'.\n"
        f"2. Answer user questions based strictly on the report context"
        f" above.\n"
