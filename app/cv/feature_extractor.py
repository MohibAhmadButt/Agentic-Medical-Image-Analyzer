...
class BiomedFeatureExtractor:
    def __init__(self, device: str = None):
        ...
        self.pathology_candidates = [
            ...
            "pneumonia or pulmonary opacity",
            "pleural effusion",
            "pulmonary edema",
    ...
    def analyze_image(self, image_input: Union[str, bytes], custom_labels: List[str] = None) -> Dict:
        image = self._load_image(image_input)
        image_tensor = self.preprocess(image).unsqueeze(0).to(self.device)

        # Modality Classification
        modality_tokens = self.tokenizer([f"a medical image of {m}" for m in self.modalities]).to(self.device)
        
        # Pathology Classification
        labels_to_test = custom_labels or self.pathology_candidates
        pathology_tokens = self.tokenizer([f"medical scan demonstrating {p}" for p in labels_to_test]).to(self.device)
        ...
        with torch.no_grad():
            ...
            mod_probs = (100.0 * image_feat @ mod_feat.T).softmax(dim=-1).cpu().numpy()[0]
            ...
            path_probs = (100.0 * image_feat @ path_feat.T).softmax(dim=-1).cpu().numpy()[0]
